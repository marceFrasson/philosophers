...
/*   Updated: 2022/09/24 13:55:48 by mfrasson         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    sleep_ms(int time)
{
    ...
    // start = current_time();
    // while (current_time() - start < time * 1000) {
    //     usleep(100);
    // }
    usleep(time * 1000);
}
...
int    time_to_stop(t_table *table)
{
    ...
    while (++i < table->number_of_philos)
    {
        ...
        // printf("%d ---- %d ---- %d\n", philo.table->times_a_philo_must_eat, philo.times_eaten, philo.index);
...
static void    *simulation(void *args)
{
    ...
    while (true)
    {
        ...
        philo->times_eaten++;
        printf("times_eaten: %d ---- philo: %d\n", philo->times_eaten, philo->index);
        pthread_mutex_unlock(philo->left_fork);
        pthread_mutex_unlock(philo->right_fork);

        // ver se ele terminou de comer
        print(philo, SLEEP);
